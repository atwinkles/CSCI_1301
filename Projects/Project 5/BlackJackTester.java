...
public class BlackJackTester extends BlackJackApp {
	...
	private static boolean testCardClass() {
        ...
        Card card1 = new Card(types[0],suits[0]);
        Card card2 = new Card(types[0],suits[0]);
        Card card3 = new Card(types[0],suits[1]);
        if (!card1.equals(card2)){
            System.out.println("Equals is broken");
             return false;
        }
	...
	public static boolean testDeckClass() {
		...
		for (Card drawnCard : drawnCards) {
			testCard = testDeck.draw(); // error here
			if(!drawnCard.equals(testCard)) {
